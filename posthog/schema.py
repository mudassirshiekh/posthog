...
class RetentionQuery(BaseModel):
    ...
    date_range: Optional[InsightDateRange] = Field(
        ...
        description=(
            ...
            " datamodel-code-generator does not generate a model with a factory.\n\nThis would cause a different"
            " serialization of an empty and a missing prop, which we don't want for stable cache keys.\n\nAs a"
            " workaround we simply set any prop to its default, so that a factory is used to instantiate an empty model"
            " for a missing prop."
        ),
    )
    filter_test_accounts: Optional[bool] = Field(
...
class StickinessQuery(BaseModel):
    ...
    stickiness_filter: Optional[StickinessFilter] = Field(
        ...
        alias="stickinessFilter",
        description=(
            "Properties specific to the stickiness insight\n\n:TRICKY: We can't set the default to an empty dict, as"
            " otherwise datamodel-code-generator does not generate a model with a factory.\n\nThis would cause a"
            " different serialization of an empty and a missing prop, which we don't want for stable cache keys.\n\nAs"
            " a workaround we simply set any prop to its default, so that a factory is used to instantiate an empty"
            " model for a missing prop."
        ),
    )
